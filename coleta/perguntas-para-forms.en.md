...
3. Suggested title: `AI Maturity Assessment - <Organization Name>`
4. Add **3 sections** (one per pillar) using `+ Add new` -> `Section`
5. For each question, add 2 elements:
   a. **Choice** (single answer) with the question text + the 6 options below
   b. **Long Text** with label `Evidence` (optional field)
6. At the end, set `Settings -> Anyone can respond` when sharing by link
7. Share the link with the team
8. When responses are ready, open `Responses -> Open in Excel` to download the .xlsx

## Fixed Options for ALL Questions (Choice / Single answer)
